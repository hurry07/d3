...
function svgWarning() {
    if (!document.createElementNS || !document.createElementNS("http://www.w3.org/2000/svg", "svg").createSVGRect)window.onload = function () {
...
function totalsToFrequencies(a, b) {
    ...
    for (var d in a) {
        var f = a[d] / c, g;
        a[d] > 0 ? g = b[d] / a[d] : g = 0, d == "null" ? e.calm = {d: null, p: f, s: null} : e.dirs.push({d: parseInt(d), p: f, s: g})
    }
    return e
}
function rollupForMonths(a, b) {
    var c = {}, d = {};
    for (var e = 10; e < 361; e += 10)c["" + e] = 0, d["" + e] = 0;
    c["null"] = 0, d["null"] = 0;
    for (var f in a.data) {
        var g = f.split(":");
        if (g.length == 1)var h = g[0]; else var i = g[0], h = g[1];
        if (b && !b[i - 1])continue;
...
function drawComplexArcs(a, b, c, d, e, f) {
    ...
    if (!1)a.append("svg:g").attr("class", "arctext").selectAll("text").data(b.dirs).enter().append("svg:text").text(d).attr("dy", "-3px").attr("transform", f);
...
function makeWindVis(a) {
    ...
    d3.json(b, function (a) {
        c = a, updatePageText(a), drawBigWindrose(a, "#windrose", "Frequency by Direction"), drawBigWindrose(a, "#windspeed", "Average Speed by Direction"), selectedMonthControl.setCallback(function () {
...
function drawBigWindrose(a, b, c) {
    ...
    if (b == "#windrose")var j = d3.range(.025, .151, .025), k = d3.range(.05, .101, .05), l = probabilityToRadiusScale, m = function (a) {
        return"" + (a * 100).toFixed(0) + "%"
    }; else var j = d3.range(5, 20.1, 5), k = d3.range(5, 15.1, 5), l = speedToRadiusScale, m = function (a) {
        return"" + a + "kts"
    };
...
function plotSmallRose(a, b) {
    ...
    for (var e = 1; e < 13; e++) {
        var f = 0;
        sum = 0;
        for (var g = 0; g < 12; g++)d[g] && (f += 1, sum += b[g][e]);
        var h = sum / f;
        c.push({d: e * 30, p: h / 100})
    }
...
function installGeoTiler() {
    if (d3.geo.tiler)console.log("d3.geo.tiler defined: not using ours. Good luck!"); else {
        d3.geo.tiler = function () {
            function h(b, c, d, e) {
                if (e >= f)return b.map(a);
                var g = -1, i = b.length, j = [
                    [],
                    [],
                    [],
                    []
                ...
                return c <<= 1, d <<= 1, {0: j[0].length && h(j[0], c, d, e), 1: j[1].length && h(j[1], c, d + 1, e), 2: j[2].length && h(j[2], c + 1, d, e), 3: j[3].length && h(j[3], c + 1, d + 1, e)}
            }
            ...
            return b.location = function (a) {
                ...
                function l(a) {
                    if (a.length)for (var b = -1, c = a.length; ++b < c;)j.push(a[b]); else for (var b = -1; ++b < 4;)a[b] && l(a[b])
                }
...
function stations(a, b) {
    function e(a, b) {
        function e(a) {
            return a = b({lon: a.value[0], lat: a.value[1]}), "translate(" + a.x + "," + a.y + ")"
        }
...
function makeMap() {
    var a = location.hash != "";
    map = po.map().container(document.getElementById("map").appendChild(po.svg("svg"))).add(po.dblclick()).add(po.drag()).add(po.arrow()).add(po.wheel().smooth(!1)).add(po.hash()).add(po.touch().rotate(!1)), a || (map.center({lon: -95, lat: 36}), map.zoom(4), !navigator.geolocation || navigator.geolocation.getCurrentPosition(function (a) {
        map.center({lon: a.coords.longitude, lat: a.coords.latitude}), map.zoom(8)
    }, function (a, b) {
    }, {maximumAge: 36e5, timeout: 1e4, enableHighAccuracy: !1})), map.add(po.image().id("osmMapnik").url(po.url("http://{S}tile.openstreetmap.org/{Z}/{X}/{Y}.png").hosts(["a.", "b.", "c.", ""]))), map.zoomRange([2, 12]);
...
function searchMap(a) {
    ...
    b = b.toUpperCase();
    var c = stationDb[b];
    c ? (map.center({lon: c[0], lat: c[1]}), map.zoom() < 8 && map.zoom(8)) : (document.forms[0].elements[0].style["background-color"] = "#d55", setTimeout(function () {
...
(function () {
    ...
    function j(a) {
        ...
        return b.add = function (a) {
            for (var d = 0; d < c.length; d++)if (c[d].listener == a)return b;
            return c.push({listener: a, on: !0}), b
        }, b.remove = function (a) {
    ...
    function bc(a, b) {
        ...
        return a.id = b, a.tween = function (b, d) {
            return arguments.length < 2 ? c[b] : (d == null ? delete c[b] : c[b] = d, a)
        }, a.ease = function (b) {
            ...
            return a.each(function (h, i, j) {
                ...
                function q(a) {
                    ...
                    if (c >= 1)return r(), bg = b, e.end.dispatch.call(l, h, i), bg = 0, 1
                }
                ...
                var k = [], l = this, m = a[j][i].delay, n = a[j][i].duration, o = l.__transition__ || (l.__transition__ = {active: 0, count: 0});
    ...
    function bn() {
        ...
        while (b)b.flush ? b = a ? a.next = b.next : bj = b.next : (c = Math.min(c, b.then + b.delay), b = (a = b).next);
        return c
    }
    ...
    function bH(a, b) {
        ...
        return f.domain = function (d) {
            ...
            return d = a.length < 2 ? [(h + i) / 2] : d3.range(h + j * g / 2, i + j / 2, j), e = 0, b = {t: "rangePoints", x: c, p: g}, f
        }, f.rangeBands = function (c, g) {
            arguments.length < 2 && (g = 0);
            var h = c[0], i = c[1], j = (i - h) / (a.length + g);
            return d = d3.range(h + j * g, i, j), e = j * (1 - g), b = {t: "rangeBands", x: c, p: g}, f
        }, f.rangeRoundBands = function (c, g) {
            arguments.length < 2 && (g = 0);
            var h = c[0], i = c[1], j = Math.floor((i - h) / (a.length + g)), k = i - h - (a.length - g) * j;
            return d = d3.range(h + Math.round(k / 2), i, j), e = Math.round(j * (1 - g)), b = {t: "rangeRoundBands", x: c, p: g}, f
    ...
    function cd(a, b) {
        if (b.length < 1 || a.length != b.length && a.length != b.length + 2)return bZ(a);
    ...
    function cp(a) {
        ...
        while (++b < c)d[b] = g + (g = co(e = f, f = a[b + 1]));
        return d[b] = g, d
    }
    ...
    function cT(a) {
        ...
        try {
            d3.event = {dx: d, dy: e}, cN[a].dispatch.apply(cO, cP)
        } finally {
            d3.event = b
        }
        b.preventDefault()
    ...
    function dk() {
        ...
        while (++b < c)da[(d = a[b]).identifier] = di(d);
        return a
    }
    ...
    function dq(a, b, c) {
        ...
        d3.event = {scale: h, translate: [e, f], transform: function (a, b) {
            a && i(a, e), b && i(b, f)
        }};
        try {
            dd.apply(de, df)
    ...
    d3.functor = function (a) {
        ...
        for (var a = -1, b = d3.min(arguments, f), c = new Array(b); ++a < b;)for (var d = -1, e, g = c[a] = new Array(e); ++d < e;)g[d] = arguments[d][a];
        return c
        ...
        function f(c, g) {
            ...
            while (++h < i)(k = j(l = c[h]))in m ? m[k].push(l)
                : m[k] = [l];
    ...
    d3.round = function (a, b) {
        ...
        var b = a.indexOf(":");
        return b < 0 ? a : {space: d3.ns.prefix[a.substring(0, b)], local: a.substring(b + 1)}
    ...
    var k = /(?:([^{])?([<>=^]))?([+\- ])?(#)?(0)?([0-9]+)?(,)?(\.[0-9]+)?([a-zA-Z%])?/, l = {g: function (a, b) {
    ...
    }}, o = w(2), p = w(3), q = {linear: function () {
        return v
    }, poly: w, quad: function () {
        return o
    }, cubic: function () {
        return p
    }, sin: function () {
        ...
        return z
    }, elastic: A, back: B, bounce: function () {
        return C
    }}, r = {"in": function (a) {
        return a
    }, out: t, "in-out": u, "out-in": function (a) {
        return u(t(a))
    }};
    d3.ease = function (a) {
        ...
        D.lastIndex = 0;
        for (d = 0; c = D.exec(b); ++d)c.index && h.push(b.substring(f, g = c.index)), i.push({i: h.length, x: c[0]}), h.push(null), f = D.lastIndex;
    ...
    var D = /[-+]?(?:\d+\.\d+|\d+\.|\.\d+|\d+)(?:[eE][-]?\d+)?/g, E = {background: 1, fill: 1, stroke: 1};
    ...
    d3.selection = function () {
        ...
        for (var e = -1, f = this.length; ++e < f;)for (var g = this[e], h = -1, i = g.length; ++h < i;)if (d = g[h])b.push(c = a.call(d, d.__data__, h)), c.parentNode = d;
        return T(b)
    }, W.attr = function (a, b) {
        function d() {
        ...
        function f(a, f) {
            ...
            if (b) {
                ...
                for (g = -1; ++g < i;)o = q[s = b.call(f, p = f[g], g)], o ? (o.__data__ = p, l[g] = o, m[g] = n[g] = null) : (m[g] = Z(p), l[g] = n[g] = null), delete q[s];
                for (g = -1; ++g < h;)r[g]in q && (n[g] = a[g])
            } else {
                for (g = -1; ++g < j;)o = a[g], p = f[g], o ? (o.__data__ = p, l[g] = o, m[g] = n[g] = null) : (m[g] = Z(p), l[g] = n[g] = null);
                for (; g < i; ++g)m[g] = Z(f[g]), l[g] = n[g] = null;
    ...
    _.append = W.append, _.insert = W.insert, _.empty = W.empty, _.select = function (a) {
        ...
        for (var d = -1, e = this.length; ++d < e;) {
            a.push(b = []);
            for (var f = this[d], g = -1, h = f.length; ++g < h;)b.push((c = f[g]) ? {node: c, delay: 0, duration: 250} : null)
        }
        return bc(a, bg || ++bf)
    };
    var bb = T([
    ...
    bb[0].parentNode = document.documentElement, d3.select = function (a) {
        return typeof a == "string" ? bb.select(a) : T([
            [a]
        ])
    ...
    be.call = W.call, d3.transition = function () {
        ...
        for (var f = -1, g = this.length; ++f < g;) {
            b.push(c = []);
            for (var h = this[f], i = -1, j = h.length; ++i < j;)(e = h[i]) && (d = a.call(e.node, e.node.__data__, i)) ? ("__data__"in e.node && (d.__data__ = e.node.__data__), c.push({node: d, delay: e.delay, duration: e.duration})) : c.push(null)
        }
        return bc(b, this.id).ease(this.ease())
    }, be.selectAll = function (a) {
        var b = [], c, d;
        typeof a != "function" && (a = Y(a));
        for (var e = -1, f = this.length; ++e < f;)for (var g = this[e], h = -1, i = g.length; ++h < i;)if (d = g[h]) {
            b.push(c = a.call(d.node, d.node.__data__, h));
            for (var j = -1, k = c.length; ++j < k;)c[j] = {node: c[j], delay: d.delay, duration: d.duration}
        }
        return bc(b, this.id).ease(this.ease())
    ...
    d3.timer = function (a, b, c) {
        ...
        d || (bj = {callback: a, then: c, delay: b, next: bj}), bk || (bl = clearTimeout(bl), bk = 1, bo(bm))
    }, d3.timer.flush = function () {
        var a, b = Date.now(), c = bj;
        while (c)a = b - c.then, c.delay || (c.flush = c.callback(a)), c = c.next;
        bn()
    };
    var bo = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || function (a) {
        setTimeout(a, 17)
    };
    ...
    var bY = {linear: bZ, "step-before": b$, "step-after": b_, basis: cf, "basis-open": cg, "basis-closed": ch, bundle: ci, cardinal: cc, "cardinal-open": ca, "cardinal-closed": cb, monotone: cr}, ck = [0, 2 / 3, 1 / 3, 0], cl = [0, 1 / 3, 2 / 3, 0], cm = [0, 1 / 6, 2 / 3, 1 / 6];
    ...
    function h(a, b) {
        for (var c = a.features, d = 0, f = c.length; d < f; d++)e(c[d].geometry, b)
    }
    ...
    d3.geo.mercator = function () {
        function d(d) {
            var e = d[0] / 360, f = -
                (Math.log(Math.tan(Math.PI / 4 + d[1] * a / 2)) / a) / 360;
        ...
        var a = 4.5, e = b(a), f = d3.geo.albersUsa(), i = {FeatureCollection: function (a) {
            var b = [], c = a.features, e = -1, f = c.length;
            while (++e < f)b.push(d(i, c[e].geometry));
            return b.join("")
        }, Feature: function (a) {
            ...
            while (++e < f)b.push(d(i, c[e]));
            return b.join("")
        }}, j = {FeatureCollection: function (a) {
            var b = 0, c = a.features, e = -1, f = c.length;
            while (++e < f)b += d(j, c[e]);
            return b
        }, Feature: function (a) {
            return d(j, a.geometry)
        }, Point: c, MultiPoint: c, LineString: c, MultiLineString: c, Polygon: function (a) {
    ...
    var f = {Feature: g, FeatureCollection: h, LineString: i, MultiLineString: j, MultiPoint: i, MultiPolygon: k, Point: l, Polygon: m};
...
org.polymaps || (org.polymaps = {}), function (a) {
    function d(b) {
        var c = b.indexOf(":");
        return c < 0 ? b : {space: a.ns[b.substring(0, c)], local: b.substring(c + 1)}
    }

    function e(a) {
        for (var b = 0; b < e.maps.length; b++)e.maps[b].resize()
    }
    ...
    function i(a, b) {
        ...
        return{x0: a.column, y0: a.row, x1: b.column, y1: b.row, dx: b.column - a.column, dy: b.row - a.row}
    }
    ...
    a.version = "2.5.0";
    var c = {x: 0, y: 0};
    a.ns = {svg: "http://www.w3.org/2000/svg", xlink: "http://www.w3.org/1999/xlink"}, a.id = function () {
        var a = 0;
        return function () {
            return++a
        }
        ...
        return arguments.length || (a = 1, c = 0, e = 0, b = 0, d = 1, f = 0), g.zoomFraction = function (e) {
            ...
            return{column: (a * g.column * i + c * g.row * i + e) * j, row: (b * g.column * i + d * g.row * i + f) * j, zoom: g.zoom - h}
        }, g.unapply = function (g) {
            var i = Math.pow(2, -g.zoom), j = Math.pow(2, g.zoom + h);
            return{column: (g.column * i * d - g.row * i * c - e * d + f * c) / (a * d - b * c) * j, row: (g.column * i * b - g.row * i * a - e * b + f * a) / (c * b - d * a) * j, zoom: g.zoom + h}
        }, g.toString = function () {
            return"matrix(" + [a * j, b * j, c * j, d * j].join(" ") + " 0 0)"
        }, g.zoomFraction(0)
    }, a.cache = function (a, b) {
        function j(a) {
            i--, b && b(a), delete e[a.key];
            if (a.next)a.next.prev = a.prev; else if (g = a.prev)g.next = null;
            if (a.prev)a.prev.next = a.next; else if (f = a.next)f.prev = null
        }
        ...
        return c.peek = function (a) {
            ...
            return j ? (j.prev && (j.prev.next = j.next, j.next ? j.next.prev = j.prev : g = j.prev, j.prev = null, j.next = f, f.prev = j, f = j), j.lock = 1, d[h] = j, j) : (j = {key: h, column: b.column, row: b.row, zoom: b.zoom, next: f, prev: null, lock: 1}, a.call(null, j, c), d[h] = e[h] = j, f ? f.prev = j : g = j, f = j, i++, j)
        }, c.unload = function (a) {
            if (a in d) {
                var b = d[a];
                return b.lock = 0, delete d[a], b.request && b.request.abort(!1) && j(b), b
            }
            return!1
        }, c.locks = function () {
            return d
        ...
        function e(e) {
            ...
            if (d)g = e.column % f, g < 0 && (g += f); else if (g < 0 || g >= f)return null;
            return b.replace(/{(.)}/g, function (b, d) {
                switch (d) {
                    ...
                    case"Y":
                        return e.row;
                    case"B":
                        var f = a.map.coordinateLocation({row: e.row, column: g, zoom: e.zoom}), h = a.map.coordinateLocation({row: e.row + 1, column: g + 1, zoom: e.zoom}), i = Math.ceil(Math.log(e.zoom) / Math.LN2);
        ...
        return a.on = function (c, d) {
            var e = b[c] || (b[c] = []);
            for (var f = 0; f < e.length; f++)if (e[f].handler == d)return a;
            return e.push({handler: d, on: !0}), a
        }, a.off = function (c, d) {
            var e = b[c];
        ...
        function e() {
            if (c >= d || !b.length)return;
            c++, b.pop()()
        }

        function f(a) {
            for (var c = 0; c < b.length; c++)if (b[c] == a)return b.splice(c, 1), !0;
            return!1
        }
        ...
        function k(d, g, h) {
            ...
            var i;
            return b.push(j), e(), {abort: k}
        }

        var b = [], c = 0, d = 6;
        return{text: h, xml: j, json: i, image: k}
    }(), a.map = function () {
        function A() {
        ...
        var d = {}, i, j, k = c, l = c, m = {x: 256, y: 256}, n = {lat: 37.76487, lon: -122.41948}, o = 12, p = 0, q = 1, r = [1, 18], s = 0, t = 1, u = 0, v = 1, w = 0, x = -180, y = 180, z = [
            {lat: f(x), lon: -Infinity},
            {lat: f(y), lon: Infinity}
        ];
        ...
        return C.setAttribute("visibility", "hidden"), C.setAttribute("pointer-events", "all"), d.container = function (a) {
            ...
            for (var a = i; a; a = a.parentNode)if (a.tabIndex >= 0)return a;
            return window
        }, d.mouse = function (b) {
            ...
            return j ? (k = j, e.remove(d)) : (C.setAttribute("width", "100%"), C.setAttribute("height", "100%"), b = C.getBBox(), k = {x: b.width, y: b.height}, e.add(d)), C.setAttribute("width", k.x), C.setAttribute("height", k.y), l = {x: k.x / 2, y: k.y / 2}, B(), d.dispatch({type: "resize"}), d
        }, d.tileSize = function (a) {
            return arguments.length ? (m = a, d.dispatch({type: "move"}), d) : m
        }, d.center = function (a) {
            return arguments.length ? (n = a, B(), d.dispatch({type: "move"}), d) : n
        }, d.panBy = function (a) {
            var b = 45 / Math.pow(2, o + p - 3), c = a.x * b, e = a.y * b;
            return d.center({lon: n.lon + (w * e - v * c) / m.x, lat: f(g(n.lat) + (w * c + v * e) / m.y)})
        }, d.centerRange = function (a) {
            return arguments.length ? (z = a, z ? (x = z[0].lat > -90 ? g(z[0].lat) : -Infinity, y = z[0].lat < 90 ? g(z[1].lat) : Infinity) : (x = -Infinity, y = Infinity), B(), d.dispatch({type: "move"}), d) : z
        }, d.zoom = function (a) {
            return arguments.length ? (o = a, A(), d.center(n)) : o + p
        }, d.zoomBy = function (a, b, c) {
            if (arguments.length < 2)return d.zoom(o + p + a);
            arguments.length < 3 && (c = d.pointLocation(b)), o = o + p + a, A();
            var e = d.locationPoint(c);
            return d.panBy({x: b.x - e.x, y: b.y - e.y})
        }, d.zoomRange = function (a) {
            return arguments.length ? (r = a, d.zoom(o + p)) : r
        }, d.extent = function (a) {
            if (!arguments.length)return[d.pointLocation({x: 0, y: k.y}), d.pointLocation({x: k.x, y: 0})];
            var b = d.locationPoint(a[0]), c = d.locationPoint(a[1]), e = Math.max((c.x - b.x) / k.x, (b.y - c.y) / k.y), f = d.pointLocation({x: (b.x + c.x) / 2, y: (b.y + c.y) / 2});
            return o = o + p - Math.log(e) / Math.LN2, A(), d.center(f)
        }, d.angle = function (a) {
            return arguments.length ? (s = a, t = Math.cos(s), u = Math.sin(s), v = Math.cos(-s), w = Math.sin(-s), B(), d.dispatch({type: "move"}), d) : s
        }, d.add = function (a) {
            return a.map(d), d
        }, d.remove = function (a) {
            return a.map(null), d
        ...
        for (var b = 0; b < e.maps.length; b++)if (e.maps[b] == a)return;
        e.maps.push(a)
    }, e.remove = function (a) {
        for (var b = 0; b < e.maps.length; b++)if (e.maps[b] == a) {
            e.maps.splice(b, 1);
            return
        }
    }, window.addEventListener("resize", e, !1), window.addEventListener("load", e, !1), a.map.locationCoordinate = function (a) {
        var b = 1 / 360;
        return{column: (a.lon + 180) * b, row: (180 - g(a.lat)) * b, zoom: 0}
    }, a.map.coordinateLocation = function (a) {
        var b = 45 / Math.pow(2, a.zoom - 3);
        return{lon: b * a.column - 180, lat: f(180 - b * a.row)}
    };
    ...
    a.layer = function (b, d) {
        ...
        function t() {
            function D(b) {
                ...
                return e < 0 && (e += d), {locationPoint: function (b) {
                    var d = a.map.locationCoordinate(b), g = Math.pow(2, c - d.zoom);
                    return{x: t.x * (g * d.column - e), y: t.y * (g * d.row - f)}
                }}
            }
            ...
            function K(a, b, c) {
                ...
                for (var h = a; h < b; h++) {
                    var i = f.load({column: h, row: c, zoom: d}, D);
                    if (!i.ready && !(i.key in F)) {
                        i.proxyRefs = {};
                        var j, k, l;
                        for (var m = 1; m <= e; m++) {
                            k = !0;
                            for (var n = 0, o = 1 << m; n <= o; n++)for (var p = 0; p <= o; p++)l = f.peek(j = {column: (h << m) + p, row: (c << m) + n, zoom: d + m}), l && l.ready ? (F[l.key] = f.load(j), l.proxyCount++, i.proxyRefs[l.key] = l) : k = !1;
                            if (k)break
                        }
                        if (!k)for (var m = 1; m <= g; m++) {
                            l = f.peek(j = {column: h >> m, row: c >> m, zoom: d - m});
                            if (l && l.ready) {
            ...
            var x = b.pointCoordinate(u, c), y = b.pointCoordinate(u, {x: l.x, y: 0}), z = b.pointCoordinate(u, l), A = b.pointCoordinate(u, {x: 0, y: l.y});
            ...
            var E = f.locks(), F = {};
            for (var G in E)E[G].proxyCount = 0;
            if (h && B > -5 && B < 3) {
                var H = x.zoom < 0 ? 1 : 1 << x.zoom;
                if (g)k(x, y, z, 0, H, K), k(z, A, x, 0, H, K); else {
            ...
            for (var G in E)if (!(G in F)) {
                var L = f.unload(G);
                L.element.parentNode.removeChild(L.element), delete L.proxyRefs
            }
            for (var G in F) {
                var L = F[G];
                L.element.parentNode != p[L.level] && (p[L.level].appendChild(L.element), e.show && e.show(L))
            }
            f.flush(), e.dispatch({type: "move"})
        }
        ...
        for (var q = -4; q <= -1; q++)p[q] = m.appendChild(a.svg("g"));
        for (var q = 2; q >= 1; q--)p[q] = m.appendChild(a.svg("g"));
        return p[0] = m.appendChild(a.svg("g")), e.map = function (a) {
            if (!arguments.length)return l;
            if (l) {
                if (l == a)return m.parentNode.appendChild(m), e;
        ...
        function d(d) {
            ...
            if (typeof c == "function") {
                ...
                g != null ? d.request = a.queue.image(e, g, function (a) {
                    delete d.request, d.ready = !0, d.img = a, e.removeAttribute("opacity"), b.dispatch({type: "load", tile: d})
                }) : (d.ready = !0, b.dispatch({type: "load", tile: d}))
            } else d.ready = !0, c != null && e.setAttributeNS(a.ns.xlink, "href", c), b.dispatch({type: "load", tile: d})
        }
                ...
                switch (a.type) {
                    case"FeatureCollection":
                        for (var i = 0; i < a.features.length; i++) {
                            ...
                            k && e.push({element: g.appendChild(k), data: j})
                        }
                        break;
                    case"Feature":
                        var k = o(a.geometry, f);
                        k && e.push({element: g.appendChild(k), data: a});
                        break;
                    default:
                        var k = o(a, f);
                        k && e.push({element: g.appendChild(k), data: {type: "Feature", geometry: a}})
                }
                d.ready = !0, e.push.apply(d.features, e), c.dispatch({type: "load", tile: d, features: e})
            }

            var g = d.element = a.svg("g");
            d.features = [], f = n(f(d).locationPoint), e != null ? d.request = b(typeof e == "function" ? e(d) : e, h) : h({type: "FeatureCollection", features: m || []})
        }
        ...
        var q = {Point: function (b, c) {
            ...
            while (++h < i)e.push((g = c(f[h])).x, ",", g.y, "L");
            e.pop();
            if (!e.length)return;
            ...
            while (++h < j) {
                ...
                while (++i < k)e.push((p = c(g[i])).x, ",", p.y, "L");
                e.pop()
            }
            if (!e.length)return;
            return d.setAttribute("d", e.join("")), d
            ...
            while (++i < l) {
                ...
                while (++j < m) {
                    ...
                    while (++k < n)e.push((p = c(h[k])).x, ",", p.y, "L");
                    e[e.length - 1] = "Z"
                }
            }
            if (!e.length)return;
        ...
        c.url = function (b) {
            ...
            if (m = a)e = null, c.tile(!1);
            return c.reload()
        }, c.clip = function (a) {
            if (!arguments.length)return f;
            f && d.removeChild(i), (f = a) && d.insertBefore(i, d.firstChild);
            var b = c.cache.locks();
            for (var e in b)f ? b[e].element.setAttribute("clip-path", h) : b[e].element.removeAttribute("clip-path");
            return c
        };
        var w = c.tile;
        c.tile = function (a) {
        ...
        return c.map = function (a) {
            ...
            return f ? a.element.setAttribute("clip-path", h) : a.element.removeAttribute("clip-path"), c.dispatch({type: "show", tile: a, features: a.features}), c
        }, c.reshow = function () {
            var a = c.cache.locks();
            for (var b in a)c.show(a[b]);
            return c
        }, c
    }, a.dblclick = function () {
        function e(a) {
        ...
        return a.zoom = function (c) {
            ...
            if (c = b)d = c.container(), d.addEventListener("dblclick", e, !1);
            return a
        }, a
    }, a.drag = function () {
        function e(a) {
            if (a.shiftKey)return;
            d = {x: a.clientX, y: a.clientY}, b.focusableParent().focus(), a.preventDefault(), document.body.style.setProperty("cursor", "move", null)
        }

        function f(a) {
            if (!d)return;
            b.panBy({x: a.clientX - d.x, y: a.clientY - d.y}), d.x = a.clientX, d.y = a.clientY
        }
        ...
        return a.map = function (d) {
            ...
            if (b = d)c = b.container(), c.addEventListener("mousedown", e, !1);
            return a
        ...
        return k.style.visibility = "hidden", k.style.top = "0px", k.style.height = "0px", k.style.width = "0px", k.style.overflowY = "scroll", j.style.height = "2000px", k.appendChild(j), document.body.appendChild(k), a.smooth = function (b) {
            ...
            if (g = b)e == "mouse" && g.on("move", i), h = g.container(), h.addEventListener("mousemove", i, !1), h.addEventListener("mousewheel", l, !1), h.addEventListener("MozMousePixelScroll", l, !1);
            return a
        }, a
    }, a.arrow = function () {
        function j(a) {
            ...
            (i || j) && h.panBy({x: i, y: j}), !d && b.left | b.right | b.up | b.down && (d = setInterval(m, f)), a.preventDefault()
        }
        ...
        function m() {
            if (!h)return;
            if (Date.now() < c + e)return;
            var a = (b.left - b.right) * g, d = (b.up - b.down) * g;
            (a || d) && h.panBy({x: a, y: d})
        }

        var a = {}, b = {left: 0, right: 0, up: 0, down: 0}, c = 0, d, e = 250, f = 50, g = 16, h, i;
        return a.map = function (b) {
            if (!arguments.length)return h;
            h && (i.removeEventListener("keypress", l, !1), i.removeEventListener("keydown", j, !1), i.removeEventListener("keyup", k, !1), i = null);
            if (h = b)i = h.focusableParent(), i.addEventListener("keypress", l, !1), i.addEventListener("keydown", j, !1), i.addEventListener("keyup", k, !1);
            return a
        }, a.speed = function (b) {
        ...
        function h() {
            if (location.hash === b)return;
            e(d, (b = location.hash).substring(1)) && g()
        }

        var a = {}, b, c = 90 - 1e-8, d, e = function (a, b) {
            var d = b.split("/").map(Number);
            if (d.length < 3 || d.some(isNaN))return!0;
            var e = a.size();
            a.zoomBy(d[0] - a.zoom(), {x: e.x / 2, y: e.y / 2}, {lat: Math.min(c, Math.max(-c, d[1])), lon: d[2]})
        }, f = function (a) {
        ...
        return a.map = function (b) {
            ...
            if (d = b)d.on("move", g), window.addEventListener("hashchange", h, !1), location.hash ? h() : g();
            return a
        }, a.parser = function (b) {
        ...
        function k(b) {
            switch (b.touches.length) {
                case 1:
                    ...
                    break;
                case 2:
                    var d = b.touches[0], f = b.touches[1], j = c.mouse(d), k = c.mouse(f), l = {x: (j.x + k.x) / 2, y: (j.y + k.y) / 2}, m = a.map.locationCoordinate(i[d.identifier]), n = a.map.locationCoordinate(i[f.identifier]), o = {row: (m.row + n.row) / 2, column: (m.column + n.column) / 2, zoom: 0}, p = a.map.coordinateLocation(o);
        ...
        return window.addEventListener("touchmove", k, !1), b.rotate = function (a) {
            ...
            if (c = a)d = c.container(), d.addEventListener("touchstart", j, !1);
            return b
        }, b
    }, a.interact = function () {
        ...
        function x(a) {
            if (a.shiftKey)return q = {x0: s.mouse(a)}, s.focusableParent().focus(), F(a)
        }
        ...
        function z(a) {
            c.setAttribute("class", "compass"), q && (q.x1 && (s.extent([s.pointLocation({x: Math.min(q.x0.x, q.x1.x), y: Math.max(q.x0.y, q.x1.y)}), s.pointLocation({x: Math.max(q.x0.x, q.x1.x), y: Math.min(q.x0.y, q.x1.y)})]), r.setAttribute("display", "none")), q = null), n && (clearInterval(n), n = 0)
        }
        ...
        function K() {
            ...
            if (k != "none") {
                ...
                if (k == "big") {
                    d = {};
                    for (var t = s.zoomRange()[0], q = 0; t <= s.zoomRange()[1]; t++, q++)(d[t] = l.appendChild(I(t))).setAttribute("transform", "translate(0," + -(q + .75) * e * .4 + ")")
                }
                var u = m == "none" ? .4 : 2;
        ...
        return c.setAttribute("class", "compass"), r.setAttribute("class", "back fore"), r.setAttribute("pointer-events", "none"), r.setAttribute("display", "none"), b.radius = function (a) {
            ...
            if (s = a)t = s.container(), t.appendChild(c), t.addEventListener("mousedown", x, !1), u = t.ownerDocument.defaultView, u.addEventListener("mousemove", y, !1), u.addEventListener("mouseup", z, !1), s.on("move", J).on("resize", J), K();
            return b
        }, b
    }, a.grid = function () {
        function f(b) {
            ...
            for (var l; (l = d.locationPoint(i).x) <= h.x; i.lon += k)g || (g = e.appendChild(a.svg("line"))), g.setAttribute("x1", l), g.setAttribute("x2", l), g.setAttribute("y1", 0), g.setAttribute("y2", h.y), g = g.nextSibling;
            for (var m; (m = d.locationPoint(i).y) <= h.y; i.lat -= k)g || (g = e.appendChild(a.svg("line"))), g.setAttribute("y1", m), g.setAttribute("y2", m), g.setAttribute("x1", 0), g.setAttribute("x2", h.x), g = g.nextSibling;
            while (g) {
                var n = g.nextSibling;
                e.removeChild(g), g = n
            }
        ...
        return e.setAttribute("class", "grid"), b.map = function (a) {
            ...
            if (d = a)d.on("move", f).on("resize", f), d.container().appendChild(e), d.dispatch({type: "move"});
            return b
        }, b
    }, a.stylist = function () {
        function f(d) {
            ...
            for (n = 0; n < f; ++n) {
                if (!(k = (i = d.features[n]).element))continue;
                j = i.data;
                for (o = 0; o < g; ++o)m = (l = b[o]).value, typeof m == "function" && (m = m.call(null, j)), m == null ? l.name.local ? k.removeAttributeNS(l.name.space, l.name.local) : k.removeAttribute(l.name) : l.name.local ? k.setAttributeNS(l.name.space, l.name.local, m) : k.setAttribute(l.name, m);
                for (o = 0; o < h; ++o)m = (l = c[o]).value, typeof m == "function" && (m = m.call(null, j)), m == null ? k.style.removeProperty(l.name) : k.style.setProperty(l.name, m, l.priority);
                if (m = e) {
        ...
        return f.attr = function (a, c) {
            return b.push({name: d(a), value: c}), f
        }, f.style = function (a, b, d) {
            return c.push({name: a, value: b, priority: arguments.length < 3 ? null : d}), f
        }, f.title = function (a) {
            return e = a, f
        }, f
    }
